...
def stance_knee_extension(
    ...
    return torch.exp(-scale * avg_error)
...
def contact_pattern_reward(
    ...
    """Simple smooth reward for single support + flight, penalize double support."""
    sensor: ContactSensor = env.scene.sensors[foot_sensor_cfg.name]
    ...
    return single_support + 0.5 * flight - 1.0 * double_support
...
def push_off_velocity_reward(
    ...
    return proj_vel * push_off_strength * scale
...
def short_contact_reward(
    ...
    return torch.exp(-scale * penalty)
...
def feet_swing_height_penalty(
    ...
    return torch.exp(-scale * torch.sum(penalty, dim=1))
